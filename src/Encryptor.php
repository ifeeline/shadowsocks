...
class Encryptor
{
    ...
    protected static $_methodSupported = array(
        ...
        'idea-cfb' => array(16, 8),
        'rc2-cfb' => array(16, 8),
        //'rc4'=> array(16, 0),      //rc4的iv长度为0，导致问题
        //'rc4-md5' => array(16, 16), //php的openssl无rc4-md5算法
    ...
    protected static function getTable($key)
    {
        ...
        $i = 1;
        while ($i < 1024) {
            $table = self::mergeSort($table, function ($x, $y) use ($ah, $al, $i, $int32Max) {
    ...
    protected static function mergeSort($array, $comparison)
    {
        if (count($array) < 2) {
            return $array;
        }
        ...
        return self::merge(
            self::mergeSort(self::slice($array, 0, $middle), $comparison),
            self::mergeSort(self::slice($array, $middle), $comparison),
            $comparison
        );
    }

    protected static function slice($table, $start, $end = null)
    {
        $table = array_values($table);
        if ($end) {
    ...
    protected static function merge($left, $right, $comparison)
    {
        $result = array();
